...
def track_visit():
    ...
    try:
        ...
        # Track unique visitors by day (without storing IP)
        today = datetime.now().strftime('%Y-%m-%d')
        redis_client.pfadd(f'unique_visitors:{today}', request.headers.get('User-Agent', '') + timestamp)
        
        # Track all-time unique visitors (without storing IP)
        redis_client.pfadd('all_unique_visitors', request.headers.get('User-Agent', '') + timestamp)
...
def admin_stats():
    ...
    try:
        ...
        today = datetime.now().strftime('%Y-%m-%d')
        unique_visitors_today = redis_client.pfcount(f'unique_visitors:{today}')
        
        # Get all-time unique visitors
        all_unique_visitors = redis_client.pfcount('all_unique_visitors')
